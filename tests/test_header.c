...
int compare_headers(const evl_header_t *a, const evl_header_t *b) {
    ...
    if (memcmp(a->salt, b->salt, EVL_SALT_SIZE) != 0) return 0;
    if (memcmp(a->file_id, b->file_id, EVL_FILE_ID_SIZE) != 0) return 0;
...
int main() {
    ...
    uint8_t buffer[EVL_HEADER_SIZE];

    memcpy(original.magic, "EVL1", 4);
    original.format_version = 1;

    for (int i = 0; i < EVL_SALT_SIZE; i++) {
    ...
    print_hex(buffer, EVL_HEADER_SIZE);
